                    ...
                    PrefilterRuleStore *pmq, const uint8_t *buf, uint32_t buflen, Packet *p)
{
    ...
    MpmCtx *sub_mpm_ctx = &rxp_ctx->mpm_hs_ctx;
    int hs_count = mpm_hs_count(sub_mpm_ctx);
    SCLogDebug("RXP search with %" PRIu32 " patterns on buf %p of length %d", hs_count, buf, buflen);
